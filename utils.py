...
def load_weights(model, snn_type: str, plant_type: str = None) -> None:
    ...
    if os.path.exists(weights_file_path):
        ...
        if weight_files:  # Check if the list is not empty
            ...
            weight_path = os.path.join(weights_file_path, weight_files[-1])  # Get the file with the highest epoch number
...
def load_weights_old(model, weights_file_path):
    # Check if weight files exist and load weights from the file with the highest epoch
    last_epoch = None
    if os.path.exists(weights_file_path):
        weight_files = [f for f in os.listdir(weights_file_path) if f.endswith('.pth')]
